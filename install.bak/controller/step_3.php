...
class ControllerStep3 extends Controller {
	...
	private function validate() {
		if (!$this->request->post['db_host']) {
			$this->error['db_host'] = '请输入主机名称!';
		}

		if (!$this->request->post['db_user']) {
			$this->error['db_user'] = '请输入资料库使用者名称!';
		}

		if (!$this->request->post['db_name']) {
			$this->error['db_name'] = '请输入资料库名称!';
		}
		
		if (!$this->request->post['username']) {
			$this->error['username'] = '请输入使用者名称!';
		}
		...
		if (!is_writable(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = '错误: 不能写入 config.php 文件请确定您已设置了正确的权限在: ' . DIR_OPENCART . 'config.php!';
		}
	
		if (!is_writable(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = '错误: 不能写入 config.php 文件请确定您已设置了正确的权限在: ' . DIR_OPENCART . 'admin/config.php!';
		}	
